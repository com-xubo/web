...
sd.debug = {
  ...
  protocol:{
    ...
    serverUrl:function(){
      ...
      if(!this.protocolIsSame(sd.para.server_url,location.href)){
        sd.log('当前server_url与页面地址协议不一致，可能会存在数据丢失的风险！');
      }
    },
    ajax:function(url){
      //埋点请求不校验
      ...
      if(!this.protocolIsSame(url,location.href)){
        sd.log('当前ajax请求与页面地址协议不一致，可能会存在请求失败的风险！');
      }
    }
  }
};
